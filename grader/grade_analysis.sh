...
for blk_file in "${blk_files[@]}"; do
  ...
  # Clean specific output files instead of the whole directory
  # so that subsequent graders find all produced files.
  rm -f "out/${blk_stem}.json" "out/${blk_stem}.md"
  ...
  chk() { echo "$check_data" | jq -r "$1" 2>/dev/null | tr -d '\r'; }
