...
check_jq() {
  if ! command -v jq &>/dev/null && ! command -v jq.exe &>/dev/null; then
    # Try finding it via where.exe on Windows
    local win_path
    win_path=$(where.exe jq 2>/dev/null | head -n1) || true
    if [[ -z "$win_path" ]]; then
      ...
      # Found it via where.exe, add its directory to PATH
      local drive=$(echo "${win_path:0:1}" | tr '[:upper:]' '[:lower:]')
      local rest=$(echo "${win_path:3}" | tr '\\' '/')
      local unix_path=""
      
      # Try different common mount points for Windows drives in bash environments
      if [[ -d "/mnt/host/${drive}" ]]; then
        unix_path="/mnt/host/${drive}/${rest}"
      elif [[ -d "/${drive}" ]]; then
        unix_path="/${drive}/${rest}"
      elif [[ -d "/mnt/${drive}" ]]; then
        unix_path="/mnt/${drive}/${rest}"
      else
        # Fallback to just flipping slashes and hope bash handles it
        unix_path="/${drive}/${rest}"
      fi
      
      local jq_dir
      jq_dir=$(dirname "$unix_path")
      export PATH="$jq_dir:$PATH"
    fi
  fi
  
  # Ensure 'jq' works even if only 'jq.exe' is in PATH
  if ! command -v jq &>/dev/null && command -v jq.exe &>/dev/null; then
    # Use a function instead of alias as aliases aren't expanded in scripts by default
    jq() { jq.exe "$@"; }
